...
- **修复**: 创建了ReadingRoom实体类，并在SeatMapper中添加了相关操作
...
### 新增文件
1. `src/main/java/com/book/manager/entity/ReadingRoom.java` - 阅览室实体类
2. `src/main/resources/templates/seat/seat-manage.html` - 座位管理页面（管理员）
3. `src/main/resources/templates/seat/room-manage.html` - 阅览室管理页面（管理员）
4. `db_update.sql` - 数据库更新脚本

### 修改文件
1. `src/main/java/com/book/manager/dao/SeatMapper.java` - 扩展了座位和阅览室的数据库操作
2. `src/main/java/com/book/manager/controller/SeatController.java` - 扩展了座位和阅览室的API接口
3. `src/main/resources/templates/seat/seat-list.html` - 座位列表改为动态获取阅览室
4. `src/main/resources/templates/index.html` - 修复语法错误，添加管理员菜单
...
1. **数据库准备**
   - 确保已执行 `book_manager.sql` 或 `db_update.sql` 脚本
   - reading_room、seat、seat_reservation 表应该已存在

2. **启动项目**
   - 使用IDE导入项目或执行 `mvn spring-boot:run`
